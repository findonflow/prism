export const cadenceGetAccountListings = `
import NFTStorefrontV2 from 0x4eb8a10cb9f87357
import NonFungibleToken from 0xNonFungibleToken
import MetadataViews from 0xMetadataViews
import FTRegistry from 0xFIND
...
access(all) struct Listings {
    ...
    access(all) let listingIds: [UInt64]

    init(validItems: [Item], invalidItems: [Item], listingIds: [UInt64]) {
...
access(all) fun main(account: Address): Listings {
    ...
    return Listings(validItems: validItems, invalidItems: invalidItems, listingIds: listingIds)
}`;
